...
/* allocate descriptor */
	toreturn=(MMQueue *)wAlloc(sizeof(MMQueue));
...
/* no onqueue callback, no ontimeout callback, no extra */
	toreturn->onqueue=0;
	toreturn->ontimeout=0;
	toreturn->extra=0;
...
MMQueue * OpenMMQueue(DWORD remotepid)
{
	...
	sprintf(timeoutname,"MMQUEUE_TO_%x_%x",curpid,queuenumber);
...
/* create events */
	...
	toreturn->timeoutevent=CreateEvent(NULL,TRUE,FALSE,timeoutname);
...
void CloseMMQueue(MMQueue * toclose)
{
	...
	if(toclose->timeoutevent)
		CloseHandle(toclose->timeoutevent);
...
{
	wCopy(buffer+destpos,bytes+srcpos,size);
	return;
}
...
DWORD WINAPI MMQueueReaderThread(void * thequeue)
{
	...
	while(curqueue->keeprunning)
	{
		...
		{
			if(WaitForSingleObject(curqueue->timeoutevent,0)!=WAIT_TIMEOUT)
			{
				if(curqueue->ontimeout)
					curqueue->ontimeout(curqueue->extra);
				else
				{
			...
			else if((!isrunning(curqueue->writerpid))||(curqueue->keeprunning==0))
			{
				curqueue->queuethreadhandle=0;
				curqueue->queuethreadid=0;
				return 0;
		...
		else if(waitresult==(WAIT_OBJECT_0+1))//written event
		{
			...
			newel=(MMQueueElement *)wAlloc(sizeof(MMQueueElement));
			...
			newel->data=(BYTE *)wAlloc(newel->size);
...
void MMDoQueue(MMQueue * onqueue,unsigned int type,unsigned int size,BYTE * data)
{
	...
	if(onqueue->isWriter)
	{
		newel=wAlloc(sizeof(MMQueueElement));
		newel->data=wAlloc(size);
...
void MMQueueAddReader(MMQueue * thisqueue,unsigned int readerpid,unsigned int queuenumber)
{
	...
	newreader->timeoutevent=CreateEvent(NULL,FALSE,FALSE,timeoutname);
...
void MMQueueRemoveReader(MMQueue * thisqueue,unsigned int readerpid)
{
	...
	while(curreader)
	{
		...
		if(curreader->readerpid==readerpid)
		{
			...
			SetEvent(curreader->timeoutevent);
			CloseHandle(curreader->timeoutevent);
